/*
    ...
    Logic from levitated.com, simulates wave propagation trougt a kinetic array of nodes
...
class Flagellum {
  ...
  float muscleRange = 3;                     // controls rotation angle of the neck
  float muscleFreq = random(0.06, 0.10);     // 
  float theta_vel;
  float theta = 180;
  float theta_friction = 0.3;
  float count = 0;
  ...
  Flagellum(String _skin) {
    ...
    // random image resize
    float scalar = random(0.05, 0.075);//0.5, 1 1.5, 1.75 controls scale of koi
